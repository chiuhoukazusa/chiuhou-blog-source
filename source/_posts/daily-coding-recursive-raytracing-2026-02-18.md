...
**v5最终方案**: `diffuse=0.4` 保证基础亮度 + `reflectivity=0.8` 保持强烈反射 + 增强环境光 ✅
...
**v4致命错误**: 尝试纯镜面反射
```cpp
// 错误尝试: Material(Vec3(1.0, 1.0, 1.0), 0.0, 1.0, 1.0)
//                                         ↑diffuse=0.0
// 问题: 无直接光照贡献

// 颜色公式:
color = direct_lighting * 0.0 + reflected_color * 1.0
      = 0 + reflected_color

// 当递归深度用尽 (depth=0) 或反射到空白区域:
reflected_color = Vec3(0, 0, 0)
→ 最终结果: 纯黑球！❌
```

**v5最终修复**: 高反射 + 适量漫反射 + 增强光照
```cpp
// 最终材质: Material(Vec3(0.95, 0.95, 0.95), 0.4, 1.0, 0.8)
//                   浅银色              ↑0.4  ↑1.0 ↑0.8
// 效果: 40%直接光照 + 80%反射 → 既有基础亮度，又有强烈镜面效果
...
// 关键教训: 镜面球需要漫反射作为"保底亮度"，防止递归终止时变黑 ✅
```
