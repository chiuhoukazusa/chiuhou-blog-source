...
**第三次修复（2026-02-20 05:40）**：

**用户反馈**：阴影和地面在天上，图片上下颠倒了
...
**错误代码**：
```cpp
py = (1.0 - 2.0 * (y + 0.5) / height) * tan(fov / 2.0)
```

**修复代码**：
```cpp
py = (2.0 * (y + 0.5) / height - 1.0) * tan(fov / 2.0);
py = -py;  // ✅ 显式翻转Y轴
```

**为什么之前没发现？**
昨天的代码没有地板球，所以没有明显的上下参考点。今天加了地板后，坐标系错误立刻暴露。
